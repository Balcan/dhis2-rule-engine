            ...
            try {
                ...
                if (Boolean.valueOf(process(rule.condition()))) {
                    ...
                    for (int j = 0; j < rule.actions().size(); j++) {
                        ...
                        if (rule.actions().get(j) instanceof RuleActionAssign && ((RuleActionAssign) rule.actions().get(j)).field().isEmpty()) {
                            valueMap.put((((RuleActionAssign) rule.actions().get(j)).content().replace("#{", "").replace("}", "")),
